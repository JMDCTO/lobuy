...
export const userRegPage = async (req, res) => {
  try {
    ...
    var date = new Date(year + "-" + month + "-" + finalDay); //insert

    const data = await userModel.insertUser(email, password, alias, date, premium); //get id

    const userFeedback = await userModel.getUserIdIntern(email, password);
    const userId = parseInt(userFeedback.rows[0]['id']); //insert login infos
